...
<template>
  ...
  <div v-if="filtroFicha && analisisEstudiantes.length > 0" class="stats-row dash-stats">
    <StatCard icon="percent" label="% Asistencia General" :value="`${porcentajeGeneral}%`" variant="verde" />
    <StatCard icon="clock" label="Tardanzas" :value="resumenAsistencia.tardanzas" variant="ambar" />
    <StatCard icon="x-circle" label="Fallas Totales" :value="resumenAsistencia.fallas" variant="rojo" />
    <StatCard v-if="estudiantesEnRiesgo.length > 0" icon="alert-triangle" label="En Riesgo" :value="estudiantesEnRiesgo.length" variant="rojo" />
  </div>
